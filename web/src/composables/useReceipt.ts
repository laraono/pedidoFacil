...
export function useReceipt() {
  ...
  function buildReceiptHtml(
    ...
    const itemRows = allItems
      .map(
        (i) =>
          `<tr><td style="padding:5px 4px;">${i.amount}x</td><td style="padding:5px 4px;">${i.name}</td><td style="padding:5px 4px;text-align:right;">R$ ${(i.unitPrice || i.price).toFixed(2)}</td><td style="padding:5px 4px;text-align:right;">R$ ${i.total.toFixed(2)}</td></tr>`,
      )
      .join("");
