...
<template>
  <div>
    <div class="mb-8 text-center">
      ...
      <p class="text-muted text-sm">Selecione os cargos que sua equipe vai usar. Pode pular e configurar depois.</p>
    </div>
    ...
    <div class="flex flex-col gap-3 mb-6">
      ...
      >
        <div class="flex items-center gap-4 p-4">
          ...
          >
            <component :is="role.icon" :size="16" :class="role.selected ? 'text-accent' : 'text-muted'" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="font-bold text-sm" :class="role.selected ? 'text-accent' : 'text-[#212121]'">{{ role.label }}</p>
            <p class="text-muted text-xs truncate">{{ role.desc }}</p>
          </div>
          <div
      ...
      >
        <div class="flex items-center gap-4 p-4">
          ...
          >
            <Tablet :size="16" :class="hasTotem ? 'text-accent' : 'text-muted'" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="font-bold text-sm" :class="hasTotem ? 'text-accent' : 'text-[#212121]'">Autoatendimento (Totem)</p>
            <p class="text-muted text-xs">Clientes fazem pedidos pelo cardápio digital</p>
          </div>
          <div
            class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors shrink-0"
