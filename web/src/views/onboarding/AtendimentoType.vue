...
<template>
  <div class="min-h-screen bg-dark-bg font-inter relative flex flex-col items-center justify-center p-4 md:p-12 overflow-hidden">
    ...
    <div class="z-10 w-full max-w-5xl text-center">
      ...
      <p class="text-gray-400 text-sm md:text-lg mb-2 font-medium">
        Selecione os modelos que deseja habilitar no seu <span class="text-white">PedidoFácil</span>.
      </p>
      <p class="text-gray-600 text-xs mb-8 md:mb-12 font-medium">
      ...
      <div class="flex flex-col gap-3 mb-12 w-full max-w-xl mx-auto text-left">
        <p class="text-xs font-black uppercase tracking-widest text-gray-500 mb-1">Selecione os modelos que utilizará</p>

        <!-- Staff role accordion cards -->
        <div
          v-for="role in staffRoles"
          :key="role.key"
          class="bg-dark-card/80 backdrop-blur-sm rounded-2xl border-2 transition-all duration-300 overflow-hidden"
          :class="role.selected ? 'border-brand-green shadow-[0_0_20px_rgba(0,210,106,0.1)]' : 'border-white/5'"
        >
          <div class="flex items-center gap-4 p-4 cursor-pointer select-none" @click="toggleStaffRole(role)">
            <div class="w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 transition-colors"
              :class="role.selected ? 'bg-brand-green/10 border-brand-green/30' : 'bg-white/5 border-white/10'">
              <component :is="role.icon" :size="18" :class="role.selected ? 'text-brand-green' : 'text-gray-400'" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="font-bold text-sm" :class="role.selected ? 'text-brand-green' : 'text-white'">{{ role.label }}</p>
              <p class="text-gray-500 text-xs truncate">{{ role.desc }}</p>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors"
                :class="role.selected ? 'bg-brand-green border-brand-green' : 'border-white/20'">
              ...
              <button
                ...
                :class="role.expanded ? 'text-brand-green' : 'text-gray-500'"
              >
          ...
          >
            <div v-if="role.expanded" class="border-t border-white/5 px-4 pb-4 overflow-hidden">
              <p class="text-[10px] font-black uppercase tracking-widest text-gray-500 mt-4 mb-3">Permissões para este cargo</p>
              <div class="flex flex-col gap-2">
                <label
                  v-for="perm in role.allPerms"
                  :key="perm.id"
                  class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all"
                  :class="perm.checked ? 'bg-brand-green/5 border-brand-green/20' : 'bg-white/3 border-white/5 hover:bg-white/5'"
                ...
                >
                  <div class="w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors"
                    :class="perm.checked ? 'bg-brand-green border-brand-green' : 'border-white/20'">
                    <Check v-if="perm.checked" :size="10" class="text-black" stroke-width="4" />
                  </div>
                  <span class="text-sm font-bold" :class="perm.checked ? 'text-brand-green' : 'text-gray-300'">{{ perm.label }}</span>
                </label>
              </div>
            </div>
          </Transition>
        ...
        <div
          @click="totens = !totens"
          class="bg-dark-card/80 backdrop-blur-sm rounded-2xl border-2 transition-all duration-300 cursor-pointer select-none"
          :class="totens ? 'border-brand-green shadow-[0_0_20px_rgba(0,210,106,0.1)]' : 'border-white/5 hover:border-white/20'"
        >
          <div class="flex items-center gap-4 p-4">
            <div class="w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 transition-colors"
              :class="totens ? 'bg-brand-green/10 border-brand-green/30' : 'bg-white/5 border-white/10'">
              <Tablet :size="18" :class="totens ? 'text-brand-green' : 'text-gray-400'" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="font-bold text-sm" :class="totens ? 'text-brand-green' : 'text-white'">Autoatendimento</p>
              <p class="text-gray-500 text-xs truncate">Totens, tablets e QR Codes para pedidos sem garçom.</p>
            </div>
            <div class="w-5 h-5 rounded border-2 flex items-center justify-center transition-colors shrink-0"
              :class="totens ? 'bg-brand-green border-brand-green' : 'border-white/20'">
      ...
      <button
        ...
        :disabled="isLoading"
        class="text-gray-500 text-sm font-bold hover:text-gray-300 transition-colors mt-4 underline underline-offset-4 disabled:opacity-40"
      >
        Pular por agora
      </button>
