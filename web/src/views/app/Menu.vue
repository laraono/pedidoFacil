...
const selectedCategory = computed(() => {
    return productsByCategory.value.find(c => c.id === activeCategoryId.value) || null;
});

const cartQuantity = computed(() => cart.value.length);
const cartTotal = computed(() => cart.value.reduce((acc, item) => acc + item.price, 0));

// Calcula o valor total do produto selecionado AGORA no modal (Tamanho + Extras * Quantidade)
...
const showToast = (msg) => {
    toastMessage.value = '';
    setTimeout(() => { toastMessage.value = msg; }, 50);
};
...
const openProductModal = (product) => {
    ...
    // Auto-selecionar o primeiro tamanho disponível, se existir
...
const addToCart = () => {
    ...
    // Preço unitário (Tamanho + Soma dos Extras)
    ...
    showToast(`${currentQuantity.value}x ${currentProduct.value.name} adicionado!`);
    closeProductModal();
};
...
const finalizeOrder = () => {
    ...
    showToast('Pedido enviado para a cozinha!'); // O toast substitui a tela de sucesso
};
...
const closeVisuals = () => {
    // Em vez de apenas fechar, removemos o parâmetro da URL para manter a navegação limpa
    isEditMode.value = false;
    router.push({ path: route.path });
};
const checkEditMode = () => {
    if (route.query.editMode === 'true') {
        // O segredo está neste pequeno delay: ele garante que o componente 
        // já existe no DOM antes de mudar o estado, forçando a Transition a rodar.
        setTimeout(() => {
...
watch(() => route.query.editMode, () => {
    checkEditMode();
});
</script>

<template>
    <div class="min-h-screen flex flex-col font-inter relative" :class="{ 'sm:pr-80': isEditMode }"
        ...
        <div class="flex flex-1 overflow-hidden relative">
            <aside class="w-1/3 sm:w-64 overflow-y-auto custom-scrollbar p-3 sm:p-4 border-r border-white/5 shrink-0">
                <button v-for="category in productsByCategory" :key="category.id"
                    ...
                    <template v-if="category.image">
                        ...
                        <div v-if="activeCategoryId === category.id"
                            class="absolute inset-0 border-[1px] rounded-2xl transition-all"
                            :style="{ borderColor: categoryColor }"></div>
                    </template>

                    <span class="relative z-10 truncate w-full tracking-wide"
            ...
            <main class="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 pb-32">
                <div v-if="selectedCategory" class="animate-fadeIn">
                    <h2 class="text-3xl font-black mb-6 pb-2 border-b border-white/10 inline-block"
                        :style="{ color: textColor }">
                        {{ selectedCategory.name }}
                    </h2>

                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                        <div v-for="product in selectedCategory.products" :key="product.id"
                            ...
                            <div v-if="product.image"
                                class="w-full h-40 rounded-2xl mb-4 bg-cover bg-center shadow-inner"
                                :style="{ backgroundImage: `url(${product.image})` }"></div>

                            <div class="flex-1 z-10">
                                <h3 class="font-bold text-xl mb-2 leading-tight" :style="{ color: textColor }">{{
                                    product.name }}</h3>
                                <p class="text-sm opacity-70 line-clamp-2" :style="{ color: textColor }">{{
                                    product.description }}</p>
                            </div>

                            <div class="flex items-center justify-between mt-6 pt-4 border-t border-white/5 z-10">
                                <div class="flex flex-col">
                                    <span v-if="product.sizes?.length > 1"
                                        class="text-[10px] opacity-70 uppercase tracking-widest font-bold mb-0.5"
                                        :style="{ color: textColor }">A partir de</span>
                                    <span class="font-black text-xl" :style="{ color: buttonColor }">
                                        {{ formatCurrency(product.sizes?.[0]?.price || 0) }}
                                    </span>
                                </div>
                                <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold transition-transform group-hover:scale-110 shadow-lg"
    ...
    <Transition enter-active-class="transition-transform duration-300 ease-out"
        enter-from-class="translate-y-24 opacity-0" leave-active-class="transition-transform duration-200 ease-in"
        leave-to-class="translate-y-24 opacity-0">
        <div v-if="cart.length > 0 && !orderEnded"
            class="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-xl px-4 z-40">
            <button @click="isCartModalOpen = true" :style="{ backgroundColor: buttonColor, color: buttonTextColor }"
                class="w-full font-black py-4 px-6 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.4)] flex items-center justify-between hover:scale-[1.02] active:scale-95 transition-all">
                <div class="flex items-center gap-3">
                    <div class="bg-black/20 px-3 py-1 rounded-full text-sm" :style="{ color: buttonTextColor }">{{
                        cartQuantity }}</div>
    ...
    <Teleport to="body">
        <Transition enter-active-class="transition duration-300" enter-from-class="opacity-0"
            ...
            <div v-if="isProductModalOpen"
                ...
                <div class="w-full sm:max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-slideUp sm:animate-none"
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6 bg-black/5">
                        ...
                        <div v-if="currentProduct?.sizes?.length > 0" class="space-y-3">
                            ...
                            <div class="flex flex-col gap-2">
                                <label v-for="size in currentProduct.sizes" :key="size.name"
                                    ...
                                    <div class="flex items-center gap-3">
                                        <input type="radio" :value="size" v-model="selectedSize" class="w-5 h-5"
                                            :style="{ accentColor: buttonColor }" />
                                        <span class="font-bold">{{ size.name }}</span>
                                    </div>
                                    <span class="font-bold"
                                        :style="{ color: selectedSize?.name === size.name ? buttonColor : textColor }">
                        ...
                        <div class="space-y-3">
                            <label class="block text-sm font-bold uppercase tracking-wider opacity-70">Alguma
                                observação?</label>
                            <textarea v-model="currentObservation" placeholder="Ex: Tirar cebola..." rows="2"
                                class="w-full bg-white/5 border border-white/10 rounded-2xl p-4 focus:outline-none transition-all resize-none placeholder:opacity-30"
                                :style="{ color: textColor, focusBorderColor: buttonColor }"></textarea>
                        </div>

                        <div
                            class="flex items-center justify-between bg-white/5 p-5 rounded-3xl border border-white/10">
                            <span class="font-bold text-lg">Quantidade</span>
                            <div class="flex items-center gap-5">
                                <button @click="currentQuantity > 1 ? currentQuantity-- : null"
                                    class="w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-sm bg-white/10"
                                    :style="{ color: textColor }">
                                    <Minus :size="20" />
                                </button>
                                <span class="font-black text-2xl w-8 text-center">{{ currentQuantity }}</span>
                                <button @click="currentQuantity++"
                                    class="w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-95 shadow-sm bg-white/10"
                                    :style="{ color: textColor }">
                                    <Plus :size="20" />
                                </button>
                    ...
                    <div class="p-6 border-t border-white/10 shrink-0">
                        <button @click="addToCart" :disabled="!selectedSize"
                            :style="{ backgroundColor: buttonColor, color: buttonTextColor }"
                            class="w-full py-5 font-black rounded-2xl shadow-xl transition-transform active:scale-95 flex items-center justify-between px-8 text-lg disabled:opacity-50">
            ...
            <div v-if="isCartModalOpen"
                ...
                <div class="w-full sm:max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-slideUp sm:animate-none"
                    ...
                    <div class="p-6 flex justify-between items-center border-b border-white/10 shrink-0">
                        ...
                        <button @click="isCartModalOpen = false"
                            class="p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors"
                            :style="{ color: textColor }">
                            <X :size="24" />
                        </button>
                    ...
                    <div class="p-4 overflow-y-auto custom-scrollbar flex-1 bg-black/5">
                        <div class="space-y-3">
                            <div v-for="(item, index) in cart" :key="item.id"
                                class="p-5 rounded-3xl border border-white/5 flex gap-4 items-center relative overflow-hidden bg-white/5">
                                <div
                                    class="bg-white/10 font-black w-10 h-10 rounded-xl flex items-center justify-center shrink-0">
                                    1x</div>
                                <div class="flex-1">
                                    <div class="flex justify-between items-start mb-1">
                                        <h4 class="font-bold text-lg leading-tight">{{ item.name }}
                                            <span class="text-sm font-medium opacity-60">({{ item.sizeName
                                            }})</span>
                                        </h4>
                                        <span class="font-black ml-2">{{ formatCurrency(item.price) }}</span>
                                    </div>
                                    <p v-if="item.obs"
                                        class="text-sm bg-black/20 inline-block px-3 py-1 rounded-lg mt-2 font-medium border border-white/5">
                                        Obs: {{ item.obs }}
                                    </p>
                                </div>
                                <button @click="removeFromCart(index)"
                                    class="text-red-400 hover:text-white hover:bg-red-500/20 p-2 rounded-xl transition-colors shrink-0">
                                    <Trash2 :size="20" />
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="p-6 border-t border-white/10 shrink-0 space-y-5">
                        ...
                        <button @click="finalizeOrder" :style="{ backgroundColor: buttonColor, color: buttonTextColor }"
                            class="w-full py-5 font-black rounded-2xl shadow-xl transition-transform active:scale-95 flex justify-center items-center gap-2 text-lg">
    ...
    <Transition appear enter-active-class="transition duration-500 ease-out"
        enter-from-class="translate-x-full opacity-0" enter-to-class="translate-x-0 opacity-100"
        leave-active-class="transition duration-400 ease-in" leave-from-class="translate-x-0 opacity-100"
        leave-to-class="translate-x-full opacity-0">
        <aside v-if="isEditMode"
            class="fixed top-0 right-0 w-full sm:w-80 h-screen bg-gray-900 border-l border-white/10 z-[100] flex flex-col shadow-2xl text-white font-inter">
            <div class="p-5 border-b border-white/10 flex items-center justify-between bg-black/20 shrink-0">
                ...
                <button @click="closeVisuals"
                    class="text-gray-400 hover:text-white transition-colors bg-white/5 p-1.5 rounded-lg">
                    <X :size="20" />
                </button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                <div
                    class="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl text-blue-400 text-sm mb-4 leading-relaxed">
                    Navegue e clique no cardápio à esquerda. As alterações abaixo aplicarão instantaneamente!
                </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <input type="color" v-model="bgColor"
                            class="h-10 w-14 rounded cursor-pointer bg-transparent border-0" />
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <input type="color" v-model="cardBg"
                            class="h-10 w-14 rounded cursor-pointer bg-transparent border-0" />
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <input type="color" v-model="buttonTextColor"
                            class="h-10 w-14 rounded cursor-pointer bg-transparent border-0" />
                    </div>
                </div>
                ...
                <div class="space-y-4 flex-1">
                    <label
                        class="flex items-center justify-between p-4 border border-white/5 rounded-2xl cursor-pointer hover:bg-white/5 transition-colors">
                        <div>
            ...
            <div class="p-5 border-t border-white/10 bg-black/20 space-y-3 shrink-0">
                <button @click="saveVisuals"
                    class="w-full bg-brand-green text-black font-black py-4 rounded-xl hover:bg-brand-green-hover transition-transform active:scale-95 shadow-lg flex justify-center items-center gap-2">
                    <CheckCircle2 :size="20" /> Salvar Aparência
                </button>
                <button @click="closeVisuals"
                    class="w-full bg-white/5 text-gray-300 font-bold py-3 rounded-xl hover:bg-white/10 transition-colors">
                    Descartar e Sair
                </button>
            </div>
        </aside>
