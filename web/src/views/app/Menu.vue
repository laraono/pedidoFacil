...
const saveAmount = (change, product, size) => {
    ...
    const existingItemIndex = pedido.value.findIndex(
        item => item.produto === product.name && item.size === size.name
    )

    if(amount.value[key] !== 0) {
        if (existingItemIndex !== -1) {
            pedido.value[existingItemIndex].amount = amount.value[key]
            pedido.value[existingItemIndex].price = amount.value[key] * size.price
        } else {
            pedido.value.push({
...
const calculateTotal = () => {
    return pedido.value.reduce((sum, item) => sum + (item.price || 0), 0)
}


const addComanda = (order) => {
    comandaStore.addComanda(order, calculateTotal())
...
<template>
    ...
    <div class="flex w-full h-lvh">
        ...
        <div v-if="hasPedido" class="fixed bottom-0 w-full bg-black p-4">
            ...
            <div class="flex justify-between w-full mb-1" v-for="item in pedido">
        ...
        <Teleport to="body">
            <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
                <div class="bg-white p-4 rounded-lg max-w-2xl w-full">
                    ...
                    <div class="flex justify-between w-full mb-1">
                        <button @click="cancelPedido" :style="{background: localStorageService.getButtonColors()}" class="text-black p-2">Cancelar</button>
                        <button @click="salvarPedido" :style="{background: localStorageService.getButtonColors()}" class="text-black p-2">Salvar Pedido</button>
                    </div>
                </div>
            </div>
        </Teleport>
