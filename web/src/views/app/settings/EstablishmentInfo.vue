...
const saveSettings = async () => {
  ...
  try {
    ...
    showToast('Erro ao salvar no banco local.', 'error');
  } finally {
  ...
  <main class="max-w-6xl mx-auto py-12 px-6 font-inter animate-fadeIn">
    ...
    <header class="flex items-center justify-between mb-10">
      <div class="flex items-center gap-4">
        <button @click="router.push('/app/dashboard')" class="p-3 bg-white/5 border border-white/10 rounded-2xl text-gray-400 hover:text-white transition-all">
          <ArrowLeft :size="20" />
        </button>
        <div>
      ...
      <section class="lg:col-span-1 space-y-6">
        <div class="bg-dark-card border border-white/5 rounded-[2.5rem] p-8 shadow-xl flex flex-col items-center">
          ...
          <div class="relative group cursor-pointer w-48 h-48 mb-6">
            <div class="relative w-full h-full bg-white/5 border-2 border-dashed border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col items-center justify-center group-hover:border-brand-green/50 transition-all">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                <input v-model="form.name" type="text" :class="{'border-red-500': errors.name}" class="admin-input w-full" placeholder="Ex: Hamburgueria 2000" />
                <p v-if="errors.name" class="text-red-500 text-[10px] font-bold ml-2 flex items-center gap-1"><AlertCircle :size="10"/> {{ errors.name }}</p>
              </div>
              <div class="space-y-2">
                <label class="text-xs font-black uppercase tracking-widest text-gray-500 ml-2">CNPJ</label>
                <input v-model="form.cnpj" type="text" :class="{'border-red-500': errors.cnpj}" class="admin-input w-full" placeholder="00.000.000/0001-00" />
                <p v-if="errors.cnpj" class="text-red-500 text-[10px] font-bold ml-2 flex items-center gap-1"><AlertCircle :size="10"/> {{ errors.cnpj }}</p>
              </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                <label class="text-xs font-black uppercase tracking-widest text-gray-500 ml-2">WhatsApp / Contato</label>
                <input v-model="form.phone" type="text" :class="{'border-red-500': errors.phone}" class="admin-input w-full" placeholder="(00) 00000-0000" />
                <p v-if="errors.phone" class="text-red-500 text-[10px] font-bold ml-2 flex items-center gap-1"><AlertCircle :size="10"/> {{ errors.phone }}</p>
              </div>
            <div class="space-y-2">
              <label class="text-xs font-black uppercase tracking-widest text-gray-500 ml-2">Sobre o Estabelecimento</label>
              <div class="relative">
                <AlignLeft class="absolute left-4 top-4 text-gray-500" :size="20" />
                <textarea v-model="form.description" rows="4" class="admin-input w-full pl-12 resize-none" placeholder="Conte um pouco sobre a história ou especialidades do seu negócio..."></textarea>
              </div>
            </div>
