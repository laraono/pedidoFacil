...
// Recebe os dados do filho e salva na store
...
<template>
  <main class="max-w-6xl mx-auto py-12 px-4">
            ...
            <td class="p-2 sm:p-4 text-center whitespace-nowrap">
              <div class="flex flex-col items-center">
                <button @click="menuStore.toggleAvailability(prod.id)"
                  ...
                  <div
                    :class="`w-10 h-5 rounded-full p-1 duration-300 ease-in-out ${prod.isAvailable ? 'bg-green-500' : 'bg-gray-300'} ${prod.deletedAt ? 'opacity-50 cursor-not-allowed' : ''}`">
                    <div
                      :class="`bg-white w-3 h-3 rounded-full shadow-md transform duration-300 ease-in-out ${prod.isAvailable ? 'translate-x-5' : 'translate-x-0'}`">
                    </div>
                  </div>
                </button>
                <span class="text-[8px] mt-1 font-semibold uppercase"
            ...
            <td class="p-2 sm:p-4 text-right whitespace-nowrap">
              <div class="flex justify-end gap-1 sm:gap-2">
                <template v-if="!prod.deletedAt">
                  <button @click="openEditModal(prod)" class="p-1.5 sm:p-2 text-blue-600 hover:bg-blue-50 rounded-lg"
                    title="Editar">
                    <Edit :size="16" class="sm:w-5 sm:h-5" />
                  </button>
                  <button @click="handleSoftDelete(prod)"
                    class="p-1.5 sm:p-2 text-orange-600 hover:bg-orange-50 rounded-lg" title="Arquivar">
                    <Archive :size="16" class="sm:w-5 sm:h-5" />
                  </button>
                </template>
                <template v-else>
                  <button @click="handleRestore(prod)" class="p-1.5 sm:p-2 text-green-600 hover:bg-green-50 rounded-lg"
                    title="Restaurar">
                    <RotateCcw :size="16" class="sm:w-5 sm:h-5" />
                  </button>
                  <button @click="handlePermanentDelete(prod)"
                    class="p-1.5 sm:p-2 text-red-600 hover:bg-red-50 rounded-lg" title="Deletar permanentemente">
    ...
    <ProductsModal :show="showModal" :isEditing="isEditing" :initialData="currentProductData"
