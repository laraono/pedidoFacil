...
<template>
  ...
  <aside
    ...
    :class="[
      ...
      isAdmin ? 'bg-emerald-950/95 border-accent/30' : 'bg-white border-[#E0E0E0]'
    ]"
  >
    <div
      class="p-5 border-b flex justify-between items-center"
      :class="isAdmin ? 'border-accent/15 bg-accent-light' : 'border-[#E0E0E0] bg-gray-50'"
    >
      <div class="flex items-center gap-2">
        <ShieldAlert v-if="isAdmin" :size="16" class="text-accent" />
        <span class="font-bold text-lg text-[#212121]">{{ isAdmin ? 'Admin' : 'Navegação' }}</span>
      </div>
      <button @click="$emit('close')" class="text-[#757575] hover:text-danger p-2 rounded hover:bg-red-400/10 transition-colors">
        <X class="w-6 h-6" />
      </button>
    </div>
    ...
    <nav class="flex-grow p-4 space-y-1 overflow-y-auto custom-scrollbar">
      ...
      <template v-if="isAdmin">
        <p class="text-[9px] font-black uppercase tracking-[0.2em] text-accent/50 px-4 pb-1 pt-2">Painel Admin</p>
        <a
          ...
          :class="route.path === item.route
            ...
            : 'text-accent/60 hover:bg-primary-dark/10 hover:text-accent'"
        >
