        ...
        <div class="p-8">
          <template v-if="currentPayment?.type === 'Dinheiro'">
            <div class="space-y-4">
              ...
              <div
                v-if="cashReceived >= (currentPayment?.amount || 0)"
              ...
              >
                <div class="flex justify-between items-center mb-3">
                  ...
                  <span class="text-2xl font-black text-accent">
                    {{
                      formatCurrency(
                        cashReceived - (currentPayment?.amount || 0),
                      )
                    }}
                  </span>
                </div>
            ...
            <div class="mt-8 flex gap-3">
              <button
                @click="$emit('cancel')"
                :disabled="isProcessing"
                class="flex-1 py-3 bg-white border border-[#E0E0E0] text-[#757575] font-black uppercase text-xs rounded hover:bg-gray-50 transition-all disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                :disabled="
                  cashReceived < (currentPayment?.amount || 0) || isProcessing
                "
                class="flex-1 py-3 bg-primary text-white font-black uppercase text-xs rounded hover:bg-primary-dark transition-all disabled:opacity-40 disabled:cursor-not-allowed flex justify-center items-center gap-2"
              >
          ...
          <template v-else>
            ...
            <div class="flex gap-3">
              ...
              <button
                @click="confirmPayment"
                :disabled="isProcessing"
                class="flex-1 py-3 bg-primary text-white font-black uppercase text-xs rounded hover:bg-primary-dark transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                <CheckCircle v-if="!isProcessing" :size="16" />
                <span v-if="isProcessing">Processando...</span
                ><span v-else>Recebido</span>
              </button>
            </div>
          </template>
        </div>
...
<script setup>
  ...
  const currentPayment = computed(
    () => props.payments[currentPaymentIndex.value],
  );
  ...
  const changeBreakdownList = computed(() =>
    Object.entries(changeBreakdown.value).map(([label, count]) => ({
      label,
      count,
    })),
  );
