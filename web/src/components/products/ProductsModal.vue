<script setup>
  ...
  const validateField = (field) => {
    if (field === "name") {
      if (!form.value.name?.trim())
        errors.value.name = "O título é obrigatório.";
  ...
  const removeSize = (index) =>
    form.value.sizes.length > 1 && form.value.sizes.splice(index, 1);
...
<template>
  ...
  >
    <div
      class="bg-white rounded shadow-2xl w-full max-w-4xl my-4 overflow-hidden flex flex-col max-h-[95vh]"
    >
      <div
      ...
      >
        ...
        <button
          @click="emit('close')"
          class="text-[#757575] hover:text-[#757575] transition-colors"
        >
          <X :size="24" />
        </button>
      </div>
      ...
      <div class="p-6 sm:p-10 overflow-y-auto custom-scrollbar">
        <div class="flex flex-col md:grid md:grid-cols-3 gap-8">
          <div class="md:col-span-1 space-y-6">
            <div>
              <label class="block text-sm font-bold text-[#757575] mb-3 ml-1"
                >Imagem do Produto</label
              >
              <div
                class="relative w-full aspect-square bg-gray-50 rounded border-2 border-dashed border-gray-200 hover:border-accent/50 transition-all overflow-hidden group cursor-pointer"
              >
                <img
                ...
                <div
                  v-else
                  class="w-full h-full flex flex-col items-center justify-center text-[#757575]"
                >
                  <ImageIcon :size="40" />
          ...
          <div class="md:col-span-2 space-y-5">
            ...
            <div class="bg-blue-50/50 p-5 rounded border border-blue-100">
              <div class="flex justify-between items-center mb-4">
                <h3
                  class="text-blue-900 font-bold text-sm uppercase tracking-wider"
                >
                  Tamanhos e Preços
                </h3>
                <BaseButton
                  variant="ghost"
                  @click="addSize"
                  class="text-sm py-2 px-3 text-[#757575]"
                >
              ...
              >
                <input
                  ...
                  class="text-[#757575] flex-1 p-3 bg-white border border-gray-200 rounded text-sm"
                />
                <div class="relative w-36">
                  <input
                    :value="currencyFormatter.format(size.price)"
                    @input="updatePrice(index, $event, 'sizes')"
                    class="text-[#757575] w-full pl-9 p-3 bg-white border border-gray-200 rounded text-sm text-right font-mono"
                  />
                </div>
                <button
                  @click="removeSize(index)"
                  class="text-danger hover:text-red-600 p-2"
                >
                  <Trash2 :size="20" />
                </button>
              </div>
      ...
      <div
        class="p-6 border-t border-gray-100 bg-gray-50 flex flex-col-reverse sm:flex-row justify-end gap-3"
      >
        <BaseButton
          variant="secondary"
          @click="emit('close')"
