  ...
  >
    <div class="flex items-center gap-3 sm:gap-5">
      <button
        ...
        :class="isAdmin ? 'text-white/60 hover:text-white hover:bg-white/10' : 'text-[#757575] hover:text-primary hover:bg-primary-light'"
      >
        <Menu class="w-7 h-7"/>
      </button>
    ...
    <div v-if="establishmentName" class="hidden md:flex items-center gap-2.5 px-4 py-1.5 bg-gray-50 border border-[#E0E0E0] rounded">
      ...
      <span class="text-[#212121] text-s font-bold max-w-[120px] truncate">{{ establishmentName }}</span>
    </div>

    <div class="flex items-center gap-3">
      <!-- Admin badge destacado -->
      <div v-if="isAdmin" class="flex items-center gap-2 px-3 py-1.5 bg-accent/15 border border-accent/40 rounded">
        <ShieldAlert class="w-4 h-4 text-accent" />
        <span class="text-xs font-black text-accent uppercase tracking-[0.15em]">Admin</span>
      </div>

      <!-- User info -->
      <div
        class="hidden sm:flex items-center gap-3 px-4 py-2 rounded border"
        :class="isAdmin ? 'bg-white/10 border-white/20' : 'bg-primary-light border-primary/20'"
      >
        <User class="w-4 h-4" :class="isAdmin ? 'text-white/70' : 'text-primary'" />
        <div class="flex flex-col leading-none">
          <span class="text-sm font-bold" :class="isAdmin ? 'text-white' : 'text-primary'">
            {{ authStore.user?.name || 'Gestor Principal' }}
          </span>
          <span v-if="!isAdmin" class="text-[10px] font-medium mt-0.5 text-primary/70">{{ roleName }}</span>
        </div>
      </div>
