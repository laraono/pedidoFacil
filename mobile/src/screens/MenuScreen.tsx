...
import { View, FlatList, TouchableOpacity, StyleSheet, StatusBar, Image, useWindowDimensions, Text, ActivityIndicator } from "react-native";
...
export default function MenuScreen() {
  ...
  const handleConfirmAdd = (selectedSize: any, quantity: number, obs: string) => {
  ...
  return (
    <SafeAreaView style={styles.container} {...panHandlers}>
                  ...
                  <View style={styles.productInfo}>
                    ...
                    <View style={styles.productFooter}>
                      <View style={styles.priceBlock}>
                        {((item.productVariations && item.productVariations.length > 1) || (item.sizes && item.sizes.length > 1)) && (
                      ...
                      <View style={styles.addBtnIcon}>
                        <Feather name="plus" size={20} color={theme.textoBotoes} />
                      </View>
                    </View>
                  </View>
                </TouchableOpacity>
      ...
      <ProductModal visible={modalVisible} product={selectedProduct} onClose={() => setModalVisible(false)} onConfirm={handleConfirmAdd} />
      <CartBottomBar />
    </SafeAreaView>
  );
}
...
const getStyles = (theme: any, cardWidth: number) =>
  StyleSheet.create({
    ...
    sidebar: { backgroundColor: theme.fundoProdutos, paddingTop: 24, alignItems: "center", width: 90, elevation: 10, shadowColor: "#000", shadowOffset: { width: 4, height: 0 }, shadowOpacity: 0.2, shadowRadius: 5, zIndex: 10 },
    sidebarTitle: { fontSize: 10, fontWeight: "900", color: theme.textoSecundario, textTransform: "uppercase", letterSpacing: 1, marginBottom: 20, textAlign: "center" },
    sidebarItem: { alignItems: "center", marginBottom: 24, width: "100%", paddingHorizontal: 4 },
    imageWrapper: { width: 60, height: 60, borderRadius: 30, padding: 3, borderWidth: 2, borderColor: "transparent", marginBottom: 10 },
    imageWrapperSelected: { borderColor: theme.corCategorias },
    sidebarImage: { width: "100%", height: "100%", borderRadius: 30 },
    activeIndicator: { position: "absolute", bottom: 0, right: 0, width: 14, height: 14, borderRadius: 7, backgroundColor: theme.corCategorias, borderWidth: 2, borderColor: theme.fundoProdutos },
    sidebarLabel: { textAlign: "center", fontWeight: "700", fontSize: 12, color: theme.textoSecundario, paddingHorizontal: 2 },
    sidebarLabelSelected: { color: theme.corTextoPrincipal, fontWeight: "900" },
    contentArea: { flex: 1, backgroundColor: theme.fundoGeral },
    categoryHeaderTitle: { fontSize: 28, fontWeight: "900", color: theme.corTextoPrincipal, paddingHorizontal: 16, paddingTop: 20, paddingBottom: 8, letterSpacing: -0.5 },
    gridContainer: { flex: 1 },
    listContainer: { padding: 8, paddingBottom: 150 },
    productCard: { width: cardWidth, backgroundColor: theme.fundoProdutos, margin: 6, borderRadius: 16, overflow: "visible", elevation: 6, shadowColor: "#000", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.3, shadowRadius: 6 },
    ...
    productTitle: { fontWeight: "900", fontSize: 16, color: theme.corTextoPrincipal, marginBottom: 4 },
    productDescription: { fontSize: 12, color: theme.textoSecundario, marginBottom: 8, lineHeight: 16 },
    ...
    startingFrom: { fontSize: 9, fontWeight: "900", textTransform: "uppercase", color: theme.textoSecundario, letterSpacing: 0.5, marginBottom: 2 },
    productPrice: { fontWeight: "900", color: theme.corBotoes, fontSize: 18, letterSpacing: -0.5 },
    addBtnIcon: { width: 32, height: 32, borderRadius: 16, backgroundColor: theme.corBotoes, justifyContent: "center", alignItems: "center" },
  });
