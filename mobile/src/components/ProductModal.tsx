...
interface ProductModalProps {
  ...
  onClose: () => void
  onConfirm: (size: SelectedSize, quantity: number, observation: string) => void
}

export default function ProductModal({ visible, product, onClose, onConfirm }: ProductModalProps) {
  ...
  const variations: ProductVariation[] = product?.productVariations || product?.sizes || [];
  ...
  useEffect(() => {
    if (visible && product) {
      ...
      if (variations.length > 0) {
        ...
        setSelectedSize({ name: "Único", price });
      }
    }
  }, [visible, product]);
      ...
      <View style={styles.overlay}>
        <View style={styles.content}>
          ...
          <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false}>
            {product.image && (
              <Image source={getFullImageUrl(product.image) ?? undefined} style={styles.productImage} resizeMode="cover" />
            )}
            ...
            <View style={styles.quantityContainer}>
              ...
              <View style={styles.quantityControls}>
                ...
                <TouchableOpacity style={styles.qtyBtn} onPress={() => setQuantity(quantity + 1)}>
                  <Feather name="plus" size={20} color={theme.corTextoPrincipal} />
                </TouchableOpacity>
              </View>
            </View>
          </ScrollView>
...
const getStyles = (theme: any) =>
  StyleSheet.create({
    ...
    content: { backgroundColor: theme.fundoProdutos, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "90%" },
    header: { flexDirection: "row", justifyContent: "space-between", padding: 20, borderBottomWidth: 1, borderBottomColor: theme.borda },
    title: { fontSize: 22, fontWeight: "900", color: theme.corTextoPrincipal },
    closeBtn: { padding: 8 },
    scrollArea: { padding: 20 },
    productImage: { width: "100%", height: 200, borderRadius: 16, marginBottom: 16 },
    description: { fontSize: 16, color: theme.textoSecundario, marginBottom: 20 },
    section: { marginBottom: 24 },
    sectionTitle: { fontSize: 14, fontWeight: "900", color: theme.textoSecundario, marginBottom: 12, textTransform: "uppercase" },
    sizeOption: { flexDirection: "row", justifyContent: "space-between", padding: 16, borderWidth: 1, borderColor: theme.borda, borderRadius: 12, marginBottom: 8 },
    sizeOptionSelected: { borderColor: theme.corBotoes, backgroundColor: theme.corBotoes + "10" },
    radioContainer: { flexDirection: "row", alignItems: "center" },
    radioOuter: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: theme.borda, marginRight: 10, justifyContent: "center", alignItems: "center" },
    radioOuterSelected: { borderColor: theme.corBotoes },
    radioInner: { width: 10, height: 10, borderRadius: 5, backgroundColor: theme.corBotoes },
    sizeName: { fontSize: 16, fontWeight: "700", color: theme.corTextoPrincipal },
    sizePrice: { fontSize: 16, fontWeight: "900", color: theme.corTextoPrincipal },
    sizePriceSelected: { color: theme.corBotoes },
    input: { backgroundColor: theme.fundoGeral, borderRadius: 12, padding: 15, color: theme.corTextoPrincipal, minHeight: 80, textAlignVertical: "top" },
    quantityContainer: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 30 },
